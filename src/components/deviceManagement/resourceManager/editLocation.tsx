...
function EditLocation(props: any) {
  async function handleEditLocation(event: any) {
    event.preventDefault();
    props.setRequestStatus(true);

    try {
      props.setError(null);
      let url = baseUrl + "edit-location";
      ...
      if (action.data.error) {
        ...
        props.setRequestStatus(false);
        props.setEditLocationDisplay(false);
        props.setEditLocationInfo({});
  ...
  return (
    <div className="device-info-container-1">
      ...
      >
        <div onClick={(event: any) => props.setEditLocationDisplay(false) & props.setError("")}>
          <CloseIcon />
        </div>
        
        <form
          id="edit-structure"
          onSubmit={(event) => handleEditLocation(event)}
          style={{
            overflowX: "auto",
          }}
                    ...
                    {title != "Type" ? (
                      title != "Booking Enabled" ? (
                        title == "_id" ? (
                          ...
                          <input type="text" placeholder={props.data[props.tableTitleExpectedNames[title]]} name={props.tableTitleExpectedNames[title]} className="input-param"></input>
                        )
                      ) : (
